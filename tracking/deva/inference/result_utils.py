...
def save_result(queue: Queue):
    while True:
        ...
        if save_the_mask:
            ...
            # custom saving for demo dataset (save id maps as .npy files)
            # if saver.dataset == 'demo':
            #     out_path = path.join(saver.output_root, 'ID_maps')
            #     os.makedirs(out_path, exist_ok=True)
            #     out_mask = mask.numpy().astype(np.uint32)
            #     np.save(path.join(out_path, frame_name[:-4] + '.npy'), out_mask)
