...
def extract_semantic_meshes(haprams, split='test'):
    os.makedirs(os.path.join(f'results/{hparams.dataset_name}/{hparams.exp_name}/semantic_mesh'), exist_ok=True)
    ...
    semantic_dir = os.path.join(hparams.root_dir, "semantic_cat")
    ...
    semantic_grid = torch.zeros((512, 128, 512, num_categories+1)).cuda()  # (x, y, z, c) # 0 for bg
    # counting the number of samples in each voxel
    semantic_grid_cnt = torch.zeros((512, 128, 512, num_categories+1), dtype=torch.int32).cuda()  # (x, y, z, c) # 0 for bg
    ...
    for img_idx in trange(len(render_traj_rays)):
        ...
        semantic_map = torch.from_numpy(np.load(os.path.join(semantic_dir, file_name+'.npy'))).long()
        grid_dim = torch.tensor([512, 128, 512])
    ...
    # print("Compute density values of each voxel!")
    ...
    #     density.cpu(), 
    #     os.path.join(f'results/{hparams.dataset_name}/{hparams.exp_name}/semantic_mesh', '{}.ply'.format("full_density")), 
    #     bbox=bbox.cpu(), 
    #     level=10
    # )
    ...
    for i in range(num_categories+1):
        ...
        convert_samples_to_ply(
            semantic_grid_i.cpu(), 
            os.path.join(f'results/{hparams.dataset_name}/{hparams.exp_name}/semantic_mesh', '{}.ply'.format(category_name)), 
            bbox=bbox.cpu(),
            level=0.5
        )
...
if __name__ == '__main__':
    hparams = get_opts()
    extract_semantic_meshes(hparams)